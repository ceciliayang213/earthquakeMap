...
public class LifeExpectancy extends PApplet {
	...
	public void setup() {
		...
		// 利用自訂方法(回傳map)來將.csv檔案的資料讀進lifeExpByCountry
		...
		countries = GeoJSONReader.loadData(this, "countries.geo.json");//unfoldingMap的方法, 會回傳list of Feature
		countryMarkers = MapUtils.createSimpleMarkers(countries);//unfoldingMap的方法, 會回傳list of Marker
	...
	private Map<String, Float> loadLifeExpectancyFromCSV(String fileName) {
		...
		String[] rows = loadStrings(fileName);//loadStrings is a PApplet built-in method
		for (String row : rows) {
			...
			//完整的資料長度應該是6, [4]是國家縮寫, [5]是平均年齡
			if (columns.length == 6 && !columns[5].equals("..")) {
				//這個作業需要比對兩個資料來源, world bank(擷取國家縮寫跟平均年齡)的資料跟地理資料(也有國家縮寫)
				//[4]就是key, 
