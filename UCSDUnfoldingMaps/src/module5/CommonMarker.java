...
public abstract class CommonMarker extends SimplePointMarker {
	...
	//如果comment掉整個方法, 就只會看到預設的marker(大小形狀一致的灰色圓點)
	//如果只是comment掉方法的內容, 就只會看到地圖, 上面沒有任何marker
	public void draw(PGraphics pg, float x, float y) {
		// For starter code just drawMaker(...)
		if (!hidden) {
			drawMarker(pg, x, y);//定義在CityMarker跟EarthquakeMarker中
			if (selected) {//此變數繼承自abstractMarker
